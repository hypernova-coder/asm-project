              ...
              <TabsContent value="professional" className="space-y-4 pb-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={formData.companyName || '__none__'}
                      onValueChange={(v) => {
                        if (v === '__none__') {
                          handleFormChange('companyName', '');
                        } else {
                          handleFormChange('companyName', v);
                        }
                      }}
                    >
                      ...
                      <SelectContent className="bg-slate-900 border-slate-600 max-h-60">
                        <SelectItem value="__none__" className="text-slate-400">-- None --</SelectItem>
                        {COMPANY_NAMES.map((name) => (
                          <SelectItem key={name} value={name}>{name}</SelectItem>
                        ))}
                        {formData.companyName && !COMPANY_NAMES.includes(formData.companyName as any) && (
                          <SelectItem value={formData.companyName} className="text-amber-400">
                            {formData.companyName} (legacy)
                          </SelectItem>
                        )}
                      </SelectContent>
                    </Select>
