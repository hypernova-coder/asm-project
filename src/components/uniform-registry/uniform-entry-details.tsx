          ...
          <table className="w-full border-collapse text-[12px] uppercase">
            <thead>
              <tr className="bg-gray-800 text-white">
                <th className="border border-gray-900 px-3 py-2 text-center font-bold">TOKEN #</th>
                <th className="border border-gray-900 px-3 py-2 text-left font-bold">DOC TYPE</th>
                <th className="border border-gray-900 px-3 py-2 text-left font-bold">DOC NUMBER</th>
                <th className="border border-gray-900 px-3 py-2 text-left font-bold">SITE NAME</th>
                <th className="border border-gray-900 px-3 py-2 text-left font-bold">TEAM LEADER</th>
                <th className="border border-gray-900 px-3 py-2 text-left font-bold min-w-[180px]">ITEMS</th>
                <th className="border border-gray-900 px-3 py-2 text-center font-bold">CREATED</th>
                <th className="border border-gray-900 px-3 py-2 text-center font-bold">RENEWAL</th>
                <th className="border border-gray-900 px-3 py-2 text-center font-bold">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {selectedEntries.map((entryItem, idx) => {
                ...
                return (
                  <tr key={entryItem.id} className={cn(isEven ? 'bg-gray-50/70' : 'bg-white')}>
                    <td className="border border-gray-400 px-3 py-2 text-center font-bold text-gray-900">
                      #{entryItem.tokenNumber}
                      {entryItem.isRenewal && (
                        <Badge className="ml-1 bg-purple-500/15 text-purple-600 border-purple-500/25 text-[9px] px-1 py-0">
                          RENEWAL
                        </Badge>
                      )}
                    </td>
                    ...
                    <td className="border border-gray-400 px-3 py-2 text-gray-700 font-mono">
                    ...
                    <td className="border border-gray-400 px-3 py-2">
                      {isEditing ? (
                        <Input
                          ...
                          className="h-7 text-[11px] uppercase bg-white border-gray-300"
                        />
                      ) : (
                        <span className="text-gray-700">{(entryItem.siteName || '—').toUpperCase()}</span>
                      )}
                    </td>
                    <td className="border border-gray-400 px-3 py-2">
                      {isEditing ? (
                        ...
                        <span className="text-gray-700">{(entryItem.teamLeaderName || '—').toUpperCase()}</span>
                      )}
                    </td>
                    <td className="border border-gray-400 px-3 py-2">
                      <div className="flex flex-wrap gap-1">
                        {(Object.keys(items) as (keyof ItemsMap)[]).map((key) => {
                          ...
                          if (isEditing) {
                            return (
                              <label key={key} className="flex items-center gap-0.5 text-[10px] cursor-pointer select-none">
                                <Checkbox
                                  checked={checked}
                                  onCheckedChange={(val) => {
                                    setEditData(prev => ({
                          ...
                          return checked ? (
                            <Badge key={key} className="bg-blue-500/15 text-blue-700 text-[9px] px-1 py-0 border border-blue-500/25">
                              {ITEM_LABELS[key]}
                            </Badge>
                          ) : null;
                        })}
                    ...
                    <td className="border border-gray-400 px-3 py-2 text-center text-gray-600 text-[10px]">
                      {formatDateShort(entryItem.createdAt)}
                    </td>
                    <td className="border border-gray-400 px-3 py-2 text-center text-[10px]">
                      {(() => {
                        const dateStr = formatDateShort(entryItem.renewalDate);
                        if (diffDays <= 0) {
                          return <span className="text-green-600 font-bold">{dateStr}</span>;
                        }
                        if (diffDays <= 30) {
                          return <span className="text-red-600 font-bold">{dateStr}</span>;
                        }
                          ...
                          <>
                            ...
                            {diffDays <= 0 && onRenew && (
                              <Button
                                ...
                                className="h-7 w-7 p-0 text-slate-500 hover:text-green-600 hover:bg-green-50"
                                title="Renew"
                              >
