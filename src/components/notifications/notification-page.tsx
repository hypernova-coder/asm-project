  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/cancellation-requests', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ employeeId: selectedEmployee.id, reason, customDate: customDate || undefined, createdById: userId }),
      });
      const data = await res.json();
      if (data.success) {
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white sm:max-w-md">
          ...
          <div className="flex flex-col gap-4 py-2">
            ...
            <div><Label>Date (defaults to today if empty)</Label><Input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} className="bg-slate-900 border-slate-700 text-white" /></div>
          </div>
          ...
          <div key={w.id} className="bg-slate-800 rounded-xl border border-slate-700 p-4 hover:bg-slate-700/50 transition-colors">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 text-[11px] text-slate-500 flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  {w.customDate ? formatDate(w.customDate) : formatDate(w.createdAt)}
                </div>
              </div>
            </div>
          </div>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white sm:max-w-md">
          ...
          <div className="flex flex-col gap-4 py-2">
            ...
            <div>
              <Label>Date (defaults to today if empty)</Label>
              <Input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} className="bg-slate-900 border-slate-700 text-white" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setCreateDialogOpen(false)} className="border-slate-600">Cancel</Button>
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/fines', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ employeeId: selectedEmployee.id, reason, amount: numAmount, customDate: customDate || undefined, createdById: userId }),
      });
      const data = await res.json();
      if (data.success) {
          ...
          <div key={f.id} className="bg-slate-800 rounded-xl border border-slate-700 p-4 hover:bg-slate-700/50 transition-colors">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 text-[11px] text-slate-500 flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  {f.customDate ? formatDate(f.customDate) : formatDate(f.createdAt)}
                </div>
              </div>
            </div>
          </div>
